...
def evaluate_rule(rule: Any, context: dict = None) -> Any:
    ...
    if isinstance(rule, (bool, int, str)):
        ...
        if 'and' in rule:
            ...
            return evaluate_rule(rule['equals'][0], context) == \
                evaluate_rule(rule['equals'][1], context)
        elif 'lower_than' in rule:
            # works if there are only two values to compare
            return evaluate_rule(rule['lower_than'][0], context) < \
                evaluate_rule(rule['lower_than'][1], context)
        elif 'prop' in rule:
            ...
            return [
                item for item in evaluate_rule(rule['filter'][0], context)
...
def main():
    ...
    for input in inputs_paths:
        ...
        result = evaluate_rule(
            rule_data["input"]["rule"],
            rule_data["input"]["context"]
        )
